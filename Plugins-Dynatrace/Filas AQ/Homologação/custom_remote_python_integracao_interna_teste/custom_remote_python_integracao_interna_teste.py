...
class IntegracaoInternaTeste(RemoteBasePlugin):
    ...
    def query(self, **kwargs):
        ...
        os.putenv("LD_LIBRARY_PATH", "/usr/lib/oracle/21/client64/lib")
        os.system("echo LD_LIBRARY_PATH")

        key = 'ORACLE_HOME'
        value = os.getenv(key)
        print("Value 'ORACLE_HOME':", value)

        key = 'LD_LIBRARY_PATH'
        value_ld = os.getenv(key)
        print("Value 'LD_LIBRARY_PATH':", value_ld)

        key = 'PATH'
        value_path = os.getenv(key)
        ...
        try:
            ...
            device = group.create_element("Teste", "Teste")
